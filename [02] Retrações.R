...
# CNT: Tabela de variações de volume (5932) ----------------------------------------------------

sidra_5932 <- get_sidra(api = "/t/5932/n1/all/v/all/p/all/c11255/all/d/v6561%201,v6562%201,v6563%201,v6564%201")
x <- 
  sidra_5932 %>% 
  as_tibble() %>%
  select(Trimestre, `Variável`, Valor, `Setores e subsetores`) %>%
  rename(t = Trimestre, Tipo = `Variável`, `Descrição` = `Setores e subsetores`, y = Valor) %>%
  mutate(tt = str_extract(t, "^\\d"), 
         aa = trimws(str_extract(t, "\\s\\d+$")), 
         t = as.yearqtr(paste(aa,tt,sep=":"), format = "%Y:%q") %>% yearquarter()) %>%
  select(t, Tipo, y, `Descrição`)

tipo_5932 <- x %>% pull(Tipo) %>% unique()
desc_5932 <- x %>% pull(`Descrição`) %>% unique()

volumes <- 
  x %>% 
  mutate(`Acrônimo` = factor(`Descrição`, levels = desc_5932) %>% 
           fct_recode("Agro"     = "Agropecuária - total",
                      "Ind"      = "Indústria - total",
                      "Extr"     = "Indústrias extrativas",
                      "Transf"   = "Indústrias de transformação", 
  ...
  group_by(t) %>%
  mutate(k = 1:n()) %>%
  relocate(t, Tipo, Setor, Nome, y, `Acrônimo`,`Descrição`, k) %>%
  ungroup() 

grupos_5932 <- volumes %>% select(k, Tipo, Setor, Nome, `Acrônimo`) %>% unique() 
...
rec_2008Q4 <- 
  ...
  select(t,y) %>%
  as_tsibble(index = t) %>%
  filter_index("2008 Q3" ~ .) %>%
  mutate(y = if_else(row_number() == 1, NA_real_, y)) %>%
  mutate(t = seq(from = 0, along.with = y)) %>%
  rename(Rec2008 = y)
...
prev_2021 <- 
  ...
  transmute(t, y = diflog(c2,T)) %>%
  filter_index("2020 Q4" ~ .) %>%
  mutate(y = if_else(row_number() == 1, NA_real_, y)) %>%
  mutate(t = seq(from = 4, along.with = y)) %>%
  rename(Prev2021 = y)

x <- 
  rec_2008Q4 %>%
  full_join(rec_2015Q1, by = "t") %>%
  full_join(rec_2020Q1, by = "t") %>%
  full_join(prev_2021,  by = "t") %>%
  mutate(Prev2021 = if_else(is.na(Prev2021) & !is.na(Rec2020), Rec2020, Prev2021)) %>%
  pivot_longer(Rec2008:Prev2021, names_to = "Rec", values_to = "y") %>%
  group_by(Rec) %>%
  mutate(y = Ind(y/100, 100, FALSE)) %>%
  pivot_wider(names_from = Rec, values_from = y) %>% 
  mutate(Prev2021 = if_else(t < 4, NA_real_, Prev2021)) %>%
  pivot_longer(Rec2008:Prev2021, names_to = "Rec", values_to = "y") %>%
  mutate(label = case_when(t == 2 & Rec == "Rec2020"  ~ "Covid-19", 
                           t == 6 & Rec == "Prev2021" ~ "Previsão \n 2021", 
                           t == 1 & Rec == "Rec2008"  ~ "Crise \n 2008-9", 
                           t == 7 & Rec == "Rec2015"  ~ "Retração\n 2015-16")) %>%
  group_by(Rec) %>%
  mutate(Ind = case_when(t == 8 & !is.na(y) ~ format(round(y, digits = 1), decimal.mark = ","), 
                         t == 4 & Rec == "Rec2020" ~ format(round(y, digits = 1), decimal.mark = ","), 
                         TRUE ~ NA_character_))

x %>%
  filter(t <= 8) %>%
  mutate(Rec = factor(Rec, levels = c("Rec2008", "Rec2015", "Rec2020", "Prev2021"))) %>%
  ggplot() + 
  geom_line(aes(t,y, color = Rec, linetype = Rec, size = Rec)) + 
  geom_point(data = x %>% filter(Rec == "Rec2020", t <= 8), mapping = aes(t,y), color = pal_c[1], size = 2.3) +
  geom_hline(yintercept = 100, size = 0.3, color = pal_c[6]) + 
  geom_text_repel(aes(x = t, y = y, label = label, color = Rec), 
                  nudge_x = c(-0.3,0.2,0.2,0), 
                  nudge_y = c(0,0,0.8,-0.3), 
                  fontface = "bold", 
                  segment.size = 0) + 
  geom_label_repel(aes(x = t, y = y, label = Ind, fill = Rec), 
                   size = 3, 
                   color = pal_c[9], 
                   fontface = "bold", 
                   nudge_y = c(0.8,-1,0.8,-1), 
                   segment.size = 0.2, 
                   segment.color = pal_c[1]) + 
  scale_y_continuous(NULL) + 
  scale_x_continuous("Trimestres após choque inicial", expand = c(0,0), breaks = seq(0,8,1)) + 
  scale_color_manual(NULL, values = c(pal_a[2], pal_v[2], pal_c[1], pal_vd[1])) + 
  scale_linetype_manual(NULL, values = c(1,1,1,2)) + 
  scale_size_manual(NULL, values = c(0.6,0.6,0.7,0.6)) + 
  scale_fill_manual(NULL, values = c(pal_a[2], pal_v[2], pal_c[1], pal_vd[1])) + 
  labs(title    = "BRASIL: RECUPERAÇÃO DO PIB EM TRÊS GRANDES RETRAÇÕES ECONÔMICAS", 
       subtitle = "Índice de volume normalizado (= 100) no trimestre imediatamente anterior ao choque", 
       caption  = "Fonte: Elaboração própria/IBGE") + 
...
x %>% 
  ...
  filter(t <= 8) %>%
  mutate(Prev2021 = if_else(t == 4, NA_real_, Prev2021)) %>%
  rename(Crise2008 = Rec2008) %>%
  data.frame() %>%
  list() %>%
  set_names(paste("Comparação entre Recessões"))
